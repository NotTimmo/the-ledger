# The Ledger — Web Edition (Supabase + Cloudflare Pages)
...
- **Cloudflare Pages** — hosts the site itself, and also runs a small serverless
  function (`functions/api/lookup.js`) that proxies the "Look up" feature's
  API calls, keeping your Anthropic API key private on the server.
...
5. Deploy. Cloudflare will give you a `*.pages.dev` URL — that's your site.
...
- The Anthropic API key never reaches the browser — it lives only in
  Cloudflare's environment variables, used server-side by `functions/api/lookup.js`.
...
- **Rate limiting**: each signed-in user is capped at 100 "Look up" calls
  ...
  Adjust `DAILY_LOOKUP_LIMIT` in `functions/api/lookup.js` if 100/day is
  too low or too generous for how you'll actually use it.
